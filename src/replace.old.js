 ...
 * TODO: Sanitise `selectors` to accept a lone string and just convert that to an arr.
 */
function RunWhenReady(selectors, code) {
	...
	// TODO: If a RunWhenReady() type observer never finds its element,
	// then it should be killed by the end of all plugin execution. This means
	// that when the observer is created, it should be added to some list of
	// running observers. It is removed from this list right before disconnect()
	// is called. I believe all calls of RunWhenReady() are only for like waiting
	// for the page to actually load, and the ones that need to run continuously
	// are declared using custom code. (Not fact checked bwomp bwomp)
}
